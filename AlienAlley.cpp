...
void must_init(bool test, const TCHAR * description)
{
	if (test) return;
	...
	StringCbPrintf(pszDest, cbDest, TEXT("Couldn't initialize %s\n"), description);
...
int main()
{
	must_init(al_init(), TEXT("allegro"));
	must_init(al_install_keyboard(), TEXT("keyboard"));

	ALLEGRO_TIMER* timer = al_create_timer(1.0 / 30.0);
	must_init(timer, TEXT("timer"));

	ALLEGRO_EVENT_QUEUE* queue = al_create_event_queue();
	must_init(queue, TEXT("queue"));

	ALLEGRO_DISPLAY* disp = al_create_display(640, 480);
	must_init(disp, TEXT("display"));

	ALLEGRO_FONT* font = al_create_builtin_font();
	must_init(font, TEXT("font"));

	must_init(al_init_image_addon(), TEXT("image addon"));
	ALLEGRO_BITMAP* mysha = al_load_bitmap("C:/Users/Samuel_Gomes/source/repos/AlienAlley/dat/gfx/alien.pcx");
	must_init(mysha, TEXT("alien"));
	...
	while (1)
	{
		...
		if (redraw && al_is_event_queue_empty(queue))
		{
			...
			//al_draw_scaled_bitmap(mysha, 0, 0, al_get_bitmap_width(mysha), al_get_bitmap_height(mysha), 100, 100, (float)al_get_bitmap_width(mysha) * 1.9f, (float)al_get_bitmap_height(mysha) * 1.9f, 0);
